...
import eater.Bus;
import eater.DisplayPanel;
import eater.EaterEmulator;
import eater.ROMLoader;
...
public class CPU {
	...
	public CPU() {
	 ...
	 public void setFlag(char flag, boolean condition) {
	...
	public void executeAddressModeFunction(AddressMode addressMode) {
		switch (addressMode) {
			case ACC, IMP -> IMP();
			case ABS -> ABS();
			case ABX -> ABX();
	...
	public void executeOpcodeFunction(OpCode opcode) {
		switch (opcode) {
			case ADC -> ADC();
			case AND -> AND();
			case ASL -> ASL();
